...
const platforms = [
  ...
  { x: 1800, y: 380, width: 150, height: 20 },
...
function spawnPlatform() {
  ...
  let groundPlatform = Number.isInteger(platforms.length / 7)
  platforms.push(groundPlatform ?{ x: lastPlatformX + 300, y: 450, width: 2000, height: 50 }: { x: getRandomInteger(lastPlatformX + 100, lastPlatformX + 400), y: getRandomInteger(200, 350), width: 150, height: 20 })
}

function spawnEnemy() {
  let lastEnemyX = enemies[enemies.length - 1].x
  enemies.push({ x: lastEnemyX + getRandomInteger(400, 700), y: 420, width: 40, height: 30, vx: 2 })
}
...
function update() {
  ...
  if (player.x > enemies[enemies.length - 1].x - 800) {
    spawnEnemy()
    spawnEnemy()
    spawnEnemy()
  }
